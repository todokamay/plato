...
def fake_runner(calls):
    def _run(input_folder, **kwargs):
        folder = Path(input_folder)
        filenames = sorted(path.name for path in folder.glob("*.mp4"))
        calls.append({"input_folder": folder, "filenames": filenames, "kwargs": kwargs})
        return {
...
def test_detects_mp4_and_ignores_non_mp4():
    root = make_root()
    calls = []
    original = watch_folder.run_auto_qc_fix
    try:
        watch = root / "watch"
        source = write(watch / "new_clip.mp4")
        write(watch / "notes.txt", b"not video")
        before = source.stat()
        watch_folder.run_auto_qc_fix = fake_runner(calls)

        payload = run_watch_cycle(options(root))
        ...
        watch_folder.run_auto_qc_fix = original
...
def test_waits_for_stable_file_and_skips_processed_duplicate():
    ...
    try:
        ...
        first = run_watch_cycle(opts, now_func=lambda: base)
        assert first["stable_count"] == 0
        assert len(calls) == 0

        second = run_watch_cycle(opts, now_func=lambda: base + timedelta(seconds=6))
        assert second["stable_count"] == 1
        assert second["processed_count"] == 1
        assert len(calls) == 1

        third = run_watch_cycle(opts, now_func=lambda: base + timedelta(seconds=12))
...
def test_dry_run_does_not_call_auto_qc():
    ...
    try:
        ...
        watch_folder.run_auto_qc_fix = should_not_run
        payload = run_watch_cycle(options(root, dry_run=True))
        state = load_state(root / "state" / "watch_state.json")
...
def test_once_mode_exits_after_one_cycle():
    ...
    try:
        ...
        watch_folder.run_auto_qc_fix = fake_runner(calls)
        cycles = run_watch(options(root, once=True), sleep_func=lambda seconds: (_ for _ in ()).throw(AssertionError("sleep should not run")))
